...
uint8_t WM8978_Write_Reg(uint8_t reg, uint16_t val) {
    ...
    i2c_byte_write(I2C1, (WM8978_ADDR << 1), RegAddr, RegValue);
...
//        ESP_LOGI(WM_TAG, "reset succeed");
    ...
    WM8978_Write_Reg(1,
                     0X1B);                            //R1,MICEN 1(MIC enabled),BIASEN 1(emu on),VMIDSEL[1:0]:11(5K) R1,MICEN设置为1(MIC使能)，BIASEN设置为1(模拟工作，VMIDSEL[1:0]设置为：11（5）)
    WM8978_Write_Reg(2,
                     0X1B0);                            //R2, ROUT1,LOUT1 output enabled(headphone),BOOSTENR,BOOSTENL enabled.
